...
// executeJSON runs the request, handles 402 x402 retry, and parses JSON.
func (c *Client) executeJSON(req *http.Request, bodyBytes []byte) (map[string]any, error) {
	...
	if resp.StatusCode == http.StatusPaymentRequired {
		return c.handle402JSON(req, bodyBytes)
	}

	return c.parseJSONResponse(resp)
}

// executeRaw runs the request, handles 402 x402 retry, and returns raw response.
func (c *Client) executeRaw(req *http.Request, bodyBytes []byte) (*http.Response, error) {
	...
	if resp.StatusCode == http.StatusPaymentRequired {
		resp.Body.Close()
		return c.handle402Raw(req, bodyBytes)
	}
...
func (c *Client) handle402JSON(orig *http.Request, bodyBytes []byte) (map[string]any, error) {
	if c.privateKey == nil {
	...
	// Read the 402 body from the original response
	// Re-issue the request to get the 402 body (orig.Body was already read)
	// We need to resend without auth first to capture 402 body
	reqCopy, err := c.cloneRequest(orig, bodyBytes)
	if err != nil {
		return nil, err
	}
	resp402, err := c.httpClient.Do(reqCopy)
	if err != nil {
		return nil, err
	}
...
func (c *Client) handle402Raw(orig *http.Request, bodyBytes []byte) (*http.Response, error) {
	if c.privateKey == nil {
		return nil, &InsufficientBalanceError{APIError{StatusCode: 402, Message: "payment required — provide WithPrivateKey for x402 payments"}}
	}
